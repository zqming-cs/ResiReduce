...
def main():
    ...
    if args.do_train:
        ...
        #     named_parameters = model.named_parameters(), op = hvd.Average) 
        ############# 
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                if step == 10 and hvd.rank() == 0 and ('resi' in args.memory or 'cprs' in args.memory):
                    optimizer.memory.print_ef(epoch, step)

                # 设置打印步骤
                if step == 10 and hvd.rank()==0:
    ...
    #         f.write(model_to_save.config.to_json_string())
            ...
            if args.do_predict and (args.local_rank == -1 or is_main_process()):
                ...
                for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=args.disable_progress_bar):
                    if len(all_results) % 1000 == 0 and hvd.rank()==0:
