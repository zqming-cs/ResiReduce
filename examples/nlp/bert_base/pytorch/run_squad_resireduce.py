...
def main():
    ...
    parser.add_argument('--compressor', default='topk',type=str, help='compressor type')
    ...
    x_time = []
    x_epoch = []
    x_epoch = range(1, int(args.num_train_epochs + 1))
    ...
    if args.do_train:
        ...
        ######## ST print bert-base parameters
        # if hvd.rank() == 0:
        #     # 获取每个层的大小和参数
        #     idx = 1
        #     for name, param in model.named_parameters():
        #         print(f"idx: {idx}, Layer: {name}, Size: {param.size()}, Parameters: {param.numel()}")
        #         idx += 1
        ...
        #     print(f"Number of parameters: {num_params / (1024 * 1024)} M")
        #     print(f"Total size of parameters (in bytes): {total_size}")
        #     print(f"Total size of parameters (in MB): {total_size / (1024 * 1024)}")
        ######## ED print bert-base parameters
    ...
    if args.do_train:
        ...
        train_sampler = RandomSampler(train_data) if hvd.size()==0 else DistributedSampler(train_data, num_replicas=hvd.size(), rank=hvd.rank())
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                # optimizer._communicator.compressor.iteration=step
                ...
                if step == 3 and hvd.rank() == 0 and ('resi' in args.memory or 'cprs' in args.memory):
                    optimizer.memory.print_ef(epoch + 1, step)
                    
                if step == 3 and hvd.rank() == 0:
    ...
    #         f.write(model_to_save.config.to_json_string())
            ...
            if args.do_predict and (args.local_rank == -1 or is_main_process()):
                ...
                if hvd.rank() == 0:
                    if args.do_train:
                        ...
                        if args.max_steps == -1:
                            dllogger.log(step=tuple(), data={"e2e_train_time": time_to_train,
                                                            ...
                                                            "training_sequences_per_second": len(train_features) / time_to_train,
                    ...
                    y_loss.append(final_loss)
                    x_time.append(time_to_infer)

                    print("y_F1: ", y_F1)
                    print("y_exact_match: ", y_exact_match)
                    print("y_loss: ", y_loss)
                    print("x_time: ", x_time)
