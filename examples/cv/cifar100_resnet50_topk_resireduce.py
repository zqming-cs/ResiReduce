...
parser.add_argument('--memory', type=str, default='residualrn50avg', help='Error Feedback')
...
def train(epoch):
    ...
    if 'avg' in args.memory:
        optimizer.memory.set_epoch(epoch)
    with tqdm(total=len(train_loader),
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # 保存输出EF信息
            if epoch + 1 == 1 and batch_idx == 10  and hvd.rank() == 0 and ('resi' in args.memory or 'cprs' in args.memory or 'tradeoff' in args.memory):
                optimizer.memory.print_ef(epoch + 1, batch_idx)
            
            if epoch + 1 == 1 and batch_idx == 10  and hvd.rank() == 0:
